...
{ 
    ...
    /// <returns></returns>
    public static OperateResult<byte[ ]> Action ( OperateResult<byte[ ]> methodOfGetPacket , Func<byte[ ] , OperateResult<byte[ ]>> methodOfCheckResponse )
    {     
        //报文获取失败则直接上抛失败的结果
        if ( !methodOfGetPacket. IsSuccess )
        {
            return methodOfGetPacket;
        }

        //报文获取成功则：调用串口核心交互流程的方法：发送、接收、校验
        var CheckRes = methodOfCheckResponse. Invoke ( methodOfGetPacket. Content );
