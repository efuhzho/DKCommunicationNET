...
internal class Hex81PacketBuilder_ACS : IPacketsBuilder_ACS
{
    ...
    {
        return _PBHelper. PacketShellBuilder ( Hex81Information. GetRanges_ACS );
    }
    ...
    public OperateResult<byte[ ]> Packet_SetAmplitude ( float UA , float UB , float UC , float IA , float IB , float IC , float IPA , float IPB , float IPC )
    {
        ...
        return _PBHelper. PacketShellBuilder ( Hex81Information. SetAmplitude_ACS , Hex81Information. SetAmplitude_ACS_Length , buffer );
    }
    ...
    public OperateResult<byte[ ]> Packet_SetFrequency ( float FreqOfAll , float FreqOfC = 0 )
    {
        ...
        data[1] = FreqOfC;
        byte[ ] buffer = _byteTransform. TransByte ( data );
        return _PBHelper. PacketShellBuilder ( Hex81Information. SetFrequency , Hex81Information. SetFrequencyLength , buffer );
    }
    ...
    public OperateResult<byte[ ]> Packet_SetPhase ( float PhaseUa , float PhaseUb , float PhaseUc , float PhaseIa , float PhaseIb , float PhaseIc )
    {
        ...
        data[5] = PhaseIc;
        byte[ ] buffer = _byteTransform. TransByte ( data );
        return _PBHelper. PacketShellBuilder ( Hex81Information. SetPhase , Hex81Information. SetPhaseLength , buffer );
    }

    public OperateResult<byte[ ]> Packet_SetWireMode ( WireMode wireMode )
    {
        byte[ ] data = new byte[1] { ( byte ) wireMode };
        return _PBHelper. PacketShellBuilder ( Hex81Information. SetWireMode , Hex81Information. SetWireModeLength , data );
    }
    ...
    {
        return _PBHelper. PacketShellBuilder ( Hex81Information. CloseACS );
    }

    public OperateResult<byte[ ]> Packet_Open ( )
    {
        return _PBHelper. PacketShellBuilder ( Hex81Information. OpenACS );
    }
    ...
    private OperateResult<byte[ ]> Packet_SetClosedLoopAndHarmonicMode ( CloseLoopMode closeLoopMode , HarmonicMode harmonicMode )
    {
        ...
        return _PBHelper. PacketShellBuilder ( Hex81Information. SetClosedLoop , Hex81Information. SetClosedLoopLength , data );
    }
    ...
    private OperateResult<byte[ ]> Packet_SetRanges_Helper ( byte rangeIndexOfACU , byte rangeIndexOfACI , byte rangeIndexOfIP )
    {
        try
        {
            ...
            return _PBHelper. PacketShellBuilder ( Hex81Information. SetRanges_ACS , Hex81Information. SetRanges_ACS_Length , data );
        }
        catch ( Exception ex )
        {
