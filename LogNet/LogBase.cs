...
namespace DKCommunicationNET. LogNet
{
    /// <summary>
    /// 日志存储类的基类，提供一些基础的服务
    /// </summary>
    ...
    public abstract class LogNetBase : IDisposable
    {
        ...
        public LogNetBase ( )
        {
            ...
            m_WaitForSave = new Queue<ZfhMessageItem> ( );
        ...
        private Queue<ZfhMessageItem> m_WaitForSave;                                          // 待存储数据的缓存
        ...
        /// 在存储到文件的时候将会触发的事件
        /// </summary>
        public event EventHandler<ZfhEventArgs> BeforeSaveToFile = null;

        private void OnBeforeSaveToFile ( ZfhEventArgs args )
        {
        ...
        private void WriteToFile ( HslMessageDegree degree , string keyWord , string text )
        {
            ...
            {
                // 需要记录数据
                ZfhMessageItem item = GetHslMessageItem ( degree , keyWord , text );
                AddItemToCache ( item );
            }
        }


        private void AddItemToCache ( ZfhMessageItem item )
        {
        ...
        private ZfhMessageItem? GetAndRemoveLogItem ( )
        {
            ZfhMessageItem? result = null;
        ...
        {
            // 获取需要存储的日志
            ZfhMessageItem? current = GetAndRemoveLogItem ( );
            // 进入文件操作的锁
            ...
            if ( !string. IsNullOrEmpty ( LogSaveFileName ) )
            {
                ...
                try
                {
                    ...
                    {
                        // 触发事件
                        OnBeforeSaveToFile ( new ZfhEventArgs ( ) { HslMessage = current } );
                ...
                catch ( Exception ex )
                {
                    ...
                    AddItemToCache ( new ZfhMessageItem ( )
                    {
        ...
        private string HslMessageFormate ( ZfhMessageItem hslMessage )
        {
            StringBuilder stringBuilder = new StringBuilder ( "\u0002" );
            stringBuilder. Append ( "[" );
        ...
        private ZfhMessageItem GetHslMessageItem ( HslMessageDegree degree , string keyWord , string text )
        {
            return new ZfhMessageItem ( )
            {
                KeyWord = keyWord ,
                Degree = degree ,
                Text = text ,
